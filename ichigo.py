...
def ichigo_filter():
    ...
    result = cv2.VideoWriter(
        "ichigo2.mp4", cv2.VideoWriter_fourcc(*"MP4V"), 20, size
    )
    ...
    with mp_face.FaceMesh(
        ...
        while True:
            ...
            obito_path = "/home/iping-s/PP/face_filter/ichigo.png"
            nose_image = cv2.imread(obito_path)
            # nose_image = cv2.cvtColor(obito, cv2.COLOR_BGR2RGB)

            image_height, image_width, _ = frame.shape

            nose_mask = np.zeros((image_height, image_width), np.uint8)
            ...
            if results.multi_face_landmarks:
                for face_landmarks in results.multi_face_landmarks:
                    ...
                    top_left = (
                        int(center_nose[0] - nose_width / 2 + nose_width / 11),
                        int(
                            center_nose[1] - nose_height / 2 - nose_height / 5
                        ),
                    )
                    """
                    bottom_right = (
                        int(center_nose[0] + nose_width / 2 - 20),
                        int(center_nose[1] + nose_height / 2 - 80),
                    )
                    """

                    bottom_right = (
                        int(face_point_list[411][0]),
                        int(face_point_list[411][1]),
                    )
                    try:
                        nose_pig = cv2.resize(
